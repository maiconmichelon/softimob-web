class ContratosPrestacaoServicoController < ApplicationController
  before_action :set_contrato_prestacao_servico, only: [:show, :edit, :update, :destroy]

  respond_to :html

  def index
    @contratos_prestacao_servico = ContratoPrestacaoServico.all
    respond_with(@contratos_prestacao_servico)
  end
